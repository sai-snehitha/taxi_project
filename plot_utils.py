...
def plot_aggregated_time_series(
    ...
    location_features = features.loc[features["pickup_location_id"] == row_id]

    # Find corresponding index for targets
    row_index = location_features.index

    if row_index.empty:
        raise ValueError(f"Row ID {row_id} does not exist in features.")

    actual_target = targets.loc[row_index]
    ...
    if predictions is not None:
        ...
        predicted_value = predictions.loc[predictions["pickup_location_id"] == row_id]
        
        if not predicted_value.empty:
            fig.add_scatter(
                x=[time_series_dates[-1]],  # Last timestamp
                y=[predicted_value.iloc[0]],  # Predicted value
                line_color="red",
                mode="markers",
                marker_symbol="x",
                marker_size=15,
