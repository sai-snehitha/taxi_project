...
def load_model_from_registry(version=None):
    ...
    loaded_model = joblib.load(Path(model_dir) / "lgb_model.pkl")

    # Ensure that if Hopsworks wraps the model, we extract the actual Scikit-learn model
    if hasattr(loaded_model, "model"):
        print("✅ Extracting actual model from wrapper...")
        return loaded_model.model  # Extract actual model
    return loaded_model
